...
The canonical public type is :class:`Plot` in ``laker.plot``. This
module preserves the legacy :class:`Visualizer` class and the
``radio_map_to_image`` / ``plot_radio_map`` / ``plot_convergence``
...
def radio_map_to_image(
    ...
    """Convert flat grid predictions to a 2-D image array."""
    return predictions.detach().cpu().numpy().reshape(grid_size, grid_size)
