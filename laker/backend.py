...
Single public class :class:`Backend` with single-word classmethods.
All env-var parsing happens once at import time. Call
:meth:`Backend.load` to re-read after mutating the environment.
"""
...
class Backend:
    """Backend configuration: device, dtype, compile, seed."""
    ...
    chunk: int = int(os.environ.get("LAKER_CHUNK_MEMORY_BUDGET", "64")) * 1024 * 1024
    chunk_off: bool = os.environ.get("LAKER_DISABLE_CHUNK", "") == "1"
    autocast_on: bool = os.environ.get("LAKER_AUTOCAST", "") == "1"
    ...
    def load(cls) -> None:
        ...
        cls.chunk = int(os.environ.get("LAKER_CHUNK_MEMORY_BUDGET", "64")) * 1024 * 1024
        cls.chunk_off = os.environ.get("LAKER_DISABLE_CHUNK", "") == "1"
        cls.autocast_on = os.environ.get("LAKER_AUTOCAST", "") == "1"
    ...
    def device_set(
        ...
        ``None`` auto-selects CUDA → MPS → CPU; otherwise accepts a
        device string or :class:`torch.device`.
        """
        if device is None:
    ...
    @classmethod
    def dtype_set(cls, dtype: torch.dtype) -> None:
        """Set the default floating-point dtype."""
        if dtype not in (torch.float16, torch.bfloat16, torch.float32, torch.float64):
            raise ValueError(f"dtype_set expects a floating dtype, got {dtype}")
        cls.dtype = dtype
        logger.info("Default dtype set to %s", dtype)

    @classmethod
    def chunk_set(cls, megabytes: int) -> None:
        """Override the chunk-memory budget in megabytes."""
        if megabytes <= 0:
            raise ValueError(f"chunk budget must be positive megabytes, got {megabytes}")
        cls.chunk = int(megabytes) * 1024 * 1024
    ...
    @classmethod
    def compile(cls, func, mode: str = "reduce-overhead"):
