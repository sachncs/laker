...
class _StreamNamespace:
    @staticmethod
    def update(regressor, x_new, y_new, forgetting_factor=1.0,
                rebuild_threshold=100):
        from laker.backend import to_tensor
        x_new = to_tensor(x_new, device=regressor.device, dtype=regressor.dtype)
        y_t = to_tensor(y_new, device=regressor.device, dtype=regressor.dtype)
        if y_t.dim() == 2:
            y_t = y_t.squeeze(-1)
        helper = StreamingUpdater(regressor.core)
        return helper.partial_fit(
            regressor, x_new, y_t, forgetting_factor, rebuild_threshold,
        )

    @staticmethod
    def path(regressor, x, y, regularizations, reuse_precond=True):
        from laker.backend import to_tensor
        ...
        if y_t.dim() == 2:
            y_t = y_t.squeeze(-1)
        helper = StreamingUpdater(regressor.core)
        return helper.fit_path(regressor, x, y_t, regularizations, reuse_precond)

    @staticmethod
    def continuation(regressor, x, y, lambda_max=None, lambda_min=None,
                      n_stages=5, reuse_precond=True):
        from laker.backend import to_tensor
        x = to_tensor(x, device=regressor.device, dtype=regressor.dtype)
        y_t = to_tensor(y, device=regressor.device, dtype=regressor.dtype)
        if y_t.dim() == 2:
            y_t = y_t.squeeze(-1)
        helper = StreamingUpdater(regressor.core)
        return helper.fit_continuation(
            regressor, x, y_t, lambda_max, lambda_min, n_stages, reuse_precond,
        )
