...
All examples use synthetic data via `laker.data.Data.field` and a
random `Position` encoder (seed 42 by default for reproducibility).
Runtime is a few seconds on a CPU.
